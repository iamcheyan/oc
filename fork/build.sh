...
build_entrypoint() {
  ...
  local tree_sitter_worker_file="${8:-}"
  local tree_sitter_worker_bunfs="${9:-}"
  local opencode_worker_path="${10:-}"
  local minify="${11:-true}"
  local web_ui_stub="${12:-}"
  ...
  (
    ...
    BUILD_TREE_SITTER_WORKER_FILE="$tree_sitter_worker_file" \
    BUILD_TREE_SITTER_WORKER_PATH="$tree_sitter_worker_bunfs" \
    BUILD_OPENCODE_WORKER_PATH="$opencode_worker_path" \
    BUILD_MINIFY="$minify" \
    BUILD_WEB_UI_STUB="$web_ui_stub" \
    bun --eval '
...
const workerPath = process.env.BUILD_OPENCODE_WORKER_PATH || "./src/cli/tui/worker.ts"
const webUiStub = process.env.BUILD_WEB_UI_STUB || ""
const entrypoints = [
  process.env.BUILD_ENTRYPOINT,
  workerPath,
...
await Bun.build({
  ...
  define: {
    ...
    ...(process.env.BUILD_TARGET.startsWith("bun-linux")
      ? { "process.env.OPENTUI_LIBC": JSON.stringify(process.env.BUILD_LIBC === "musl" ? "musl" : "glibc") }
      : {}),
  },
}).then((result) => {
...
main() {
  ...
  local tree_sitter_worker_file="opentui-tree-sitter-worker.js"
  local bunfs_root="/\$bunfs/root/"
  if [ "$PLATFORM_OS" = "windows" ]; then
    bunfs_root="B:/~BUN/root/"
  fi
  # Embed worker at a stable bunfs path (matches packages/opencode/script/build.ts).
  # Do not realpath node_modules symlinks into ../../ outside /$bunfs/root/.
  local tree_sitter_worker_bunfs="${bunfs_root}${tree_sitter_worker_file}"

  cat > "$WEB_UI_STUB" <<'EOF'
export default null
EOF

  build_entrypoint "$PKG_DIR" "./src/index.ts" "$out_original_bin" "$version" "$channel" "$libc" "$migrations_json" \
    "$tree_sitter_worker_file" "$tree_sitter_worker_bunfs" "$worker_path" "true" "$WEB_UI_STUB"
  build_entrypoint "$VIM_DIR" "./src/index.ts" "$out_vim_bin" "$version" "$channel" "$libc" "$migrations_json" \
    "$tree_sitter_worker_file" "$tree_sitter_worker_bunfs" "$worker_path" "false" "$WEB_UI_STUB"
